...
Vsys = 243. # km/s
savepath = '../ngc253/muse/2024July2/fits2_total/'
which_cube = 'se'
reorder = True
vels = True
rms = True
...
if rms == True:
    ...
    add_rms(which_cube=which_cube, infile='%sfits2_reordered.txt' % savepath,
                    err_infile='%sfits2_err_reordered.txt' % savepath,
                    outfile='%sfits2_reordered.txt' % savepath, 
                    err_outfile='%sfits2_err_reordered.txt' % savepath)
    
if BIC == True:
    _ = calc_BIC(infile='%sfits2_reordered.txt' % savepath, num_obs=150, free_params=12)
